"""Teleoperación gestual del SO-ARM100, versión 14.

Parte de teleop_v13.py, la versión presentada en la defensa, que se conserva
sin cambios en entrega/teleoperacion/. El algoritmo de seguimiento es el
mismo: ángulos de MediaPipe, filtros One Euro, zonas muertas, signos,
ganancias, límites y horizonte de 40 ms. Cambia sólo cómo empieza a mandar:

  [V14-1] Lee la postura real del brazo en JOINT_STATES_TOPIC. Si no llega
          en SYNC_WAIT s, no arranca: sin saber dónde está el brazo cualquier
          primera orden puede ser un tirón (bloqueo 6 de docs/09).
  [V14-2] Al calibrar con [C] y al reanudar tras [P], la orden parte de la
          postura medida y no de la última enviada. Así se puede pausar,
          mover el brazo con MoveIt y reanudar sin salto.
  [V14-3] Tras calibrar o reanudar entra en SINCRONIZANDO: la velocidad se
          limita a SYNC_VEL hasta que la orden alcanza el gesto del operador
          (error < SYNC_TOL en todas las articulaciones). Después vuelve el
          tope normal MAX_JOINT_VEL.
  [V14-4] En pausa no se publica nada, ni brazo ni pinza, igual que en v13.

Las marcas [V14-n] señalan cada cambio respecto de v13.
"""
import json
import math
import os
...
def main(args=None):
    ...
    WIN = "SO-ARM100 v14 | arranque y reanudacion seguros"
    ...
    print("\n" + "="*72)
    print("v14 | [C]calibrar [1-5]invertir [TAB]sel [+/-]ganancia")
    ...
    try:
        while rclpy.ok() and cap.isOpened():
            ...
            # HUD
            cv2.putText(disp, f"v14 ARRANQUE SEGURO | {arm_side.upper()}",
