...
def settings(env=None):
    ...
    elif camera_raw.lower().startswith(('http://', 'https://', 'rtsp://')):
        camera = camera_raw        # cámara por red: DroidCam, IP Webcam, RTSP
    else:
        raise ValueError('SOARM_CAMERA debe ser un número de /dev/video o una URL http(s):// o rtsp://.')
    ...
    if (isinstance(camera, int) and camera < 0) or width <= 0 or height <= 0:
