#!/usr/bin/env python3
"""Ejecuta teleop_v14.py con las conexiones del modo elegido en scripts/soarm.sh.
...
velocidad máxima, archivo de configuración y, en v14, el tópico de estados
...
def main():
    values = settings()
    original = Path(__file__).resolve().with_name('teleop_v14.py')
    spec = importlib.util.spec_from_file_location('soarm_v14', original)
    ...
    if es_url(values['CAMERA_INDEX']):
        ...
        print('SO-ARM100 v14 | cámara por red:', values['CAMERA_INDEX'])
    Path(values['CONFIG_FILE']).parent.mkdir(parents=True, exist_ok=True)
    print('SO-ARM100 v14 | configuración:', values)
    module.main()
